...
export default function FinanceIncomePage() {
  ...
  const [items, setItems] = useState<IncomeItem[]>(initialData);
  ...
  const openMenu = (event: React.MouseEvent<HTMLElement>, item: IncomeItem) => {
      ...
      <Dialog open={dialog.value} onClose={dialog.onFalse} maxWidth="sm" fullWidth>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  select
                  fullWidth
                  label={t('finance.income.currency')}
                  value={form.currency}
        ...
        <DialogActions>
          <Button onClick={dialog.onFalse} color="inherit">
            {t('finance.income.cancel')}
          </Button>
          <Button onClick={handleSave} variant="contained" disabled={!canSave}>
            {t('finance.income.save')}
          </Button>
        </DialogActions>
      </Dialog>

      <Dialog open={deleteDialog.value} onClose={deleteDialog.onFalse} maxWidth="xs" fullWidth>
        <DialogTitle>{t('finance.income.deleteConfirm')}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            {t('finance.income.deleteHint')}
          </Typography>
        </DialogContent>
        <DialogActions>
