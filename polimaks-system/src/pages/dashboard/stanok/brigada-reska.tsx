...
import data from 'src/data/stanok-brigada-reska.json';
...
const STORAGE_KEY = 'stanok-brigada-reska';
const WORKER_STORAGE_KEY = 'staff-workers';

export default function BrigadaReskaPage() {
  const { t } = useTranslate('pages');
  const { machineId } = useParams();

  const storageKey = machineId ? `${STORAGE_KEY}-${machineId}` : STORAGE_KEY;

  const title = `${t('brigadaPage.title')} - ${t('reskaPage.title')} | ${CONFIG.appName}`;
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
            <Box>
              <Typography variant="h4">
                {t('brigadaPage.title')} ({t('reskaPage.title')})
              </Typography>
