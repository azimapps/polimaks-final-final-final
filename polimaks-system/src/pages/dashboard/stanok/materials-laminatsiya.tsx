...
import machineSeed from 'src/data/stanok-laminatsiya.json';
...
const STORAGE_KEY = 'stanok-laminatsiya';
...
export default function MaterialsLaminatsiyaPage() {
  ...
  const rows = useMemo<MaterialRow[]>(() => {
    ...
    const unknownLabel = t('laminatsiyaMaterialsPage.unknown');
    ...
    const handlePlyonka = () => {
      ...
      txs.forEach((tx) => {
        if (tx.type !== 'out' || tx.machineType !== 'laminatsiya' || tx.machineId !== machineId)
          return;
  ...
  const heading = t('laminatsiyaMaterialsPage.title', {
    machine: machine?.name || machineId || t('laminatsiyaMaterialsPage.unknown'),
  });
  const pageTitle = `${heading} | ${CONFIG.appName}`;
  const languageLabel =
    machine?.language_code ? t(`languages.${machine.language_code}`) : '-';
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
            <Box>
              ...
              <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
                {t('laminatsiyaMaterialsPage.subtitle')}
              </Typography>
            </Box>
            <Button
              variant="outlined"
              startIcon={<Iconify icon="eva:arrow-ios-back-fill" width={18} />}
              onClick={() => navigate(paths.dashboard.stanok.laminatsiya)}
            >
              {t('laminatsiyaMaterialsPage.back')}
            </Button>
          </Stack>

          {!machine ? (
            <Card sx={{ p: 3 }}>
              <Typography variant="body1">{t('laminatsiyaMaterialsPage.notFound')}</Typography>
            </Card>
          ) : (
            <>
              <Card sx={{ p: 3 }}>
                <Typography variant="h6">{t('laminatsiyaMaterialsPage.summaryTitle')}</Typography>
                <Divider sx={{ my: 2 }} />
                <Stack
                  ...
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Box>
                      ...
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                        {t('laminatsiyaPage.languageCode')}: {languageLabel}
                      </Typography>
                    </Box>
                  </Stack>
                  <Divider flexItem orientation="vertical" sx={{ display: { xs: 'none', sm: 'block' } }} />
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    {t('laminatsiyaMaterialsPage.machineId')}: {machine.id}
                  </Typography>
                </Stack>
              </Card>
                    ...
                    <TableHead>
                      <TableRow>
                        <TableCell sx={{ minWidth: 120 }}>
                          {t('laminatsiyaMaterialsPage.table.date')}
                        </TableCell>
                        <TableCell sx={{ minWidth: 140 }}>
                          {t('laminatsiyaMaterialsPage.table.material')}
                        </TableCell>
                        <TableCell sx={{ minWidth: 220 }}>
                          {t('laminatsiyaMaterialsPage.table.item')}
                        </TableCell>
                        <TableCell sx={{ minWidth: 140 }}>
                          {t('laminatsiyaMaterialsPage.table.amount')}
                        </TableCell>
                        <TableCell>{t('laminatsiyaMaterialsPage.table.note')}</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {rows.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={5}>
                            ...
                            >
                              <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                                {t('laminatsiyaMaterialsPage.empty')}
                              </Typography>
                            </Box>
                          </TableCell>
                        </TableRow>
                          ...
                          <TableRow key={row.id} hover>
                            ...
                            <TableCell>
                              <Stack direction="row" alignItems="center" spacing={1}>
                                ...
                                {row.info.length > 0 && (
                                  <Tooltip title={t('laminatsiyaMaterialsPage.info')}>
                                    <IconButton
                                      size="small"
                                      onClick={(event) => handleOpenInfo(event, row)}
                                      aria-label={t('laminatsiyaMaterialsPage.info')}
                                    >
