...
import machineSeed from 'src/data/stanok-reska.json';
...
export default function MaterialsReskaPage() {
  ...
  const rows = useMemo<MaterialRow[]>(() => {
    ...
    const handlePlyonka = () => {
      ...
      txs.forEach((tx) => {
        if (tx.type !== 'out' || tx.machineType !== 'reska' || tx.machineId !== machineId) return;
    ...
    const handleSuyuqKraska = () => {
      ...
      txs.forEach((tx) => {
        if (tx.type !== 'out' || tx.machineType !== 'reska' || tx.machineId !== machineId)
          return;
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
            <Box>
              ...
              <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
                {t('reskaMaterialsPage.subtitle')}
              </Typography>
            </Box>
            <Button
              variant="outlined"
              startIcon={<Iconify icon="eva:arrow-ios-back-fill" width={18} />}
              onClick={() => navigate(paths.dashboard.stanok.reska)}
            >
              {t('reskaMaterialsPage.back')}
            </Button>
          </Stack>

          {!machine ? (
            <Card sx={{ p: 3 }}>
              <Typography variant="body1">{t('reskaMaterialsPage.notFound')}</Typography>
            </Card>
          ) : (
            <>
              <Card sx={{ p: 3 }}>
                <Typography variant="h6">{t('reskaMaterialsPage.summaryTitle')}</Typography>
                <Divider sx={{ my: 2 }} />
                <Stack
                  ...
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Box>
                      ...
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                        {t('reskaPage.languageCode')}: {languageLabel}
                      </Typography>
                    </Box>
                  </Stack>
                  <Divider flexItem orientation="vertical" sx={{ display: { xs: 'none', sm: 'block' } }} />
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    {t('reskaMaterialsPage.machineId')}: {machine.id}
                  </Typography>
                </Stack>
              </Card>
                    ...
                    <TableHead>
                      <TableRow>
                        <TableCell sx={{ minWidth: 120 }}>
                          {t('reskaMaterialsPage.table.date')}
                        </TableCell>
                        <TableCell sx={{ minWidth: 140 }}>
                          {t('reskaMaterialsPage.table.material')}
                        </TableCell>
                        <TableCell sx={{ minWidth: 220 }}>
                          {t('reskaMaterialsPage.table.item')}
                        </TableCell>
                        <TableCell sx={{ minWidth: 140 }}>
                          {t('reskaMaterialsPage.table.amount')}
                        </TableCell>
                        <TableCell>{t('reskaMaterialsPage.table.note')}</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {rows.length === 0 ? (
