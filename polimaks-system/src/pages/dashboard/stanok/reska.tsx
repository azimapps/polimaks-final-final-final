...
import data from 'src/data/stanok-reska.json';
...
export default function ReskaPage() {
  ...
  const title = `${t('reskaPage.title')} | ${CONFIG.appName}`;
  ...
  return (
    <>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}>
        <Stack spacing={3}>
          <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
            <Box>
              <Typography variant="h4">{t('reskaPage.title')}</Typography>
              <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
                {t('reskaPage.subtitle')}
              </Typography>
            </Box>
          ...
          <Card>
            <TableContainer>
              <Table size="medium">
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ width: 200 }}>{t('reskaPage.languageCode')}</TableCell>
                    <TableCell>{t('reskaPage.name')}</TableCell>
      ...
      <Dialog open={dialog.value} onClose={dialog.onFalse} maxWidth="xs" fullWidth>
        <DialogTitle>{editing ? t('reskaPage.edit') : t('reskaPage.add')}</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2} sx={{ pt: 1 }}>
            <TextField
              autoFocus
              select
              label={t('reskaPage.languageCode')}
      ...
      <Dialog open={deleteDialog.value} onClose={deleteDialog.onFalse} maxWidth="xs" fullWidth>
        <DialogTitle>{t('reskaPage.deleteConfirm')}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            {t('reskaPage.deleteHint')}
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      <ActionsMenu
        ...
        labels={{
          ...
          brigada: t('reskaPage.brigada'),
          materials: t('reskaPage.materials'),
        }}
      />
    </>
  );
